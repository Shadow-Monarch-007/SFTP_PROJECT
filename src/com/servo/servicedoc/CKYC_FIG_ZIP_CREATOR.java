 ...
 */
@TimerService
public class CKYC_FIG_ZIP_CREATOR {

//    @Resource
//    private SRVTimerServiceConfig Timer_Service_Id;
    @PostConstruct
    public void loadConfigurations(Connection con1) {
        System.out.println("CKYC_FIG_ZIP_CREATOR Loaded successfully");
    }
...
//    }
    ...
    public void execute(Connection con) throws Exception {
        try {
            System.out.println("INSIDE EXECUTE FIG_CKYC" + con);
                    ...
                    if (zipFileData.size() > 0 && zipBodyData.size() > 0) {
                        ...
                        if (zipFileData.get("ID_PROOF_NUMBER").toString().length() == 12) {
                            ...
                            if (!folderCreationStatus) {
                                ...
                                File finalFold = new File(System.getProperty("user.dir") + File.separator + "DMS_ARCHIVAL" + File.separator + "CKYC_FINAL" + File.separator + finalFolderName);
                            ...
                            try {
                                Files.move(Paths.get(System.getProperty("user.dir") + File.separator + "DMS_ARCHIVAL" + File.separator + "FIG" + File.separator + "PRE_CIF_ZIP" + File.separator + String.valueOf(cifConData.get("CIFID")) + ".zip"), Paths.get(System.getProperty("user.dir") + File.separator + "DMS_ARCHIVAL" + File.separator + "CKYC_FINAL" + File.separator + finalFolderName + File.separator + String.valueOf(cifConData.get("CIFID")) + ".zip"), StandardCopyOption.REPLACE_EXISTING);
...
//                         | 
                ...
                if (!finalDataForBody.equals("")) {
                    ...
                    String fileName = System.getProperty("user.dir") + File.separator + "DMS_ARCHIVAL"
                            + File.separator + "CKYC_FINAL" + File.separator + finalFolderName
...
//            String finalHeader = "10|" + seqNo + "|IN2260|01|" + size + "|" + formatter.format(new Date()) + "|V1.2|01||||";
    ...
    public HashMap<String, Object> getZipBodyData(Connection con, String cif, String Pinstid) {
        ...
        try {
            query = "select * from CKYC_EXTRACT_ZIP_V2 where STATUS is null and CUSTOMERID = '" + cif + "' and PINSTID ='" + Pinstid + "'";
    ...
    public HashMap<String, Object> getZipFileData(Connection con, String cif, String Pinstid) {
        ...
        HashMap<String, Object> zipCIFData = new HashMap<>();
        try {
            query = "select PINSTID,CIFID,PHOTOGRAPH_NAME,ID_NAME,ID_PROOF_NUMBER,ID_PROOF_TYPE,FOLDER_PATH from FIG_CIF_ZIP where PROCESSED is null and CIFID = '" + cif + "' and PINSTID ='" + Pinstid + "'";
            pstmt = con.prepareCall(query);
            rs = pstmt.executeQuery();
...
//                     updateFILEProcessedStatus(con, rs.getString("CIFID"), rs.getString("PINSTID"));
    ...
    public void updateFinalStatusUpdate(Connection con, String refNo, String CIF, String PINSTID) {
        ...
        try {
            String query = "";
            PreparedStatement pstmt = null;
            query = "UPDATE FIG_CIF_ZIP_PRE SET CKYC_REF_NO ='" + refNo + "' WHERE CIFID = '" + CIF + "' and PINSTID = '" + PINSTID + "'";
            pstmt = con.prepareStatement(query);
            pstmt.executeUpdate();
        } catch (Exception ex) {
            if (con != null) {
    ...
    public void updateFILEProcessedStatus(Connection con, String CIF, String PINSTID) {
        ...
        try {
            String query = "";
            PreparedStatement pstmt = null;
            query = "UPDATE FIG_CIF_ZIP SET PROCESSED='Y' WHERE CIFID = '" + CIF + "' and PINSTID = '" + PINSTID + "'";
            pstmt = con.prepareStatement(query);
            pstmt.executeUpdate();
        } catch (Exception ex) {
            if (con != null) {
    ...
    public void updateFILERemarks(Connection con, String columnName, String CIF, String PINSTID) {
        ...
        try {
            String query = "";
            PreparedStatement pstmt = null;
            query = "UPDATE FIG_CIF_ZIP SET REMARKS = '" + columnName + " is empty" + "',PROCESSED='Y' WHERE CIFID = '" + CIF + "' and PINSTID = '" + PINSTID + "'";
            pstmt = con.prepareStatement(query);
            pstmt.executeUpdate();
        } catch (Exception ex) {
            if (con != null) {
    ...
    public List<HashMap<String, Object>> getZipCIFData(Connection con) {
        ...
        String getzipstatus = "{call FIG_CIF_ZIP_CREATOR(?)}";
        ...
        try {
            ...
            System.err.println("Cheking the count in FIG_CIF_ZIP_PRE is greator than 5000");
            switch (picStatus) {
                case "Y": {
                    System.err.println("FIG_CIF_ZIP_PRE is greator than 5000 proceeding for zip");
                    try {
                        query = "select * from FIG_CIF_ZIP_PRE where PROCESSED = 'Y' and DATA_PROCESSED = 'Y' and CKYC_REF_NO is null and rownum < 5000";
                        pstmt = con.prepareCall(query);
                        System.out.println("INSIDE FIG_CIF_ZIP_PRE QUERY ---" + query);
                        rs = pstmt.executeQuery();
                        while (rs.next()) {
                            HashMap<String, Object> data = new HashMap<>();
                            ResultSetMetaData rsmd = rs.getMetaData();
                ...
                case "N": {
                    System.err.println("FIG_CIF_ZIP_PRE is less than 5000 Skipping the zip");
                    break;
                }
            }
