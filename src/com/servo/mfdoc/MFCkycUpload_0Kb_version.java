 ...
 */
@TimerService
public class MFCkycUpload_0Kb_version {
    ...
    public void execute(Connection con) throws Exception {
        ...
        String getDMSUploadSql = "{call getCKYCUploadCase(?,?)}";
        try {
...
//                        } else 
                ...
                if (file_front.exists() && file_back.exists()) {
                    files.add(fileA);
                    files.add(fileB);
                    System.out.println("[MFCKYC-UPLOAD]---BOTH FRONT AND BACK ID PROOFS ARE VAILABLE: ");
                    ...
                    // Neither file exists
                    System.out.println("[MFCKYC-UPLOAD]---NEITHER FRONT NOR BACK ID PROOFS ARE AVAILABLE.");
        ...
        try {
            ...
            if (updateFlag(con, dataMap.get("PROCESSINSTANCEID").toString(), "UPD", "")) {
                ...
                try {
                    ...
                    System.out.println("[MFCKYC-UPLOAD]--EXCEPTION OCCURED: " + e.getMessage());
                    updateFlag(con, dataMap.get("PROCESSINSTANCEID").toString(), "EXC", e.getMessage());
                }
                ...
                switch (d_status) {
                    case "Y": {
                        if (directoryToDelete.exists()) {
                            System.out.println("[MFCKYC-UPLOAD]--CKYC DOCUMENT DELETE STATUS IS SET AS YES --Directory Found -- Deleting Existing Folder After upload to sftp: EXT-" + deletecustomerID);
                            deleteDirectory(directoryToDelete);
                        } else {
                            System.out.println("[MFCKYC-UPLOAD]--CKYC DOCUMENT DELETE STATUS IS SET AS YES --Directory does not exist.");
                        }
                        break;
                    }
                    case "N": {
                        System.out.println("[MFCKYC-UPLOAD]--CKYC DOCUMENT DELETE STATUS IS SET AS NO -- KEEPING THE CKYC DOCUMENTS.");
                        break;
                    }
                    default: {
                        if (directoryToDelete.exists()) {
