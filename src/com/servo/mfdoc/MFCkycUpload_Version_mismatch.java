...
@TimerService
public class MFCkycUpload_Version_mismatch {
    ...
    public void execute(Connection con) throws Exception {
        ...
        String getDMSUploadSql = "{call MF_CKYC_VMM(?,?)}";
        try {
            ...
            while (rs.next()) {
                ...
                System.out.println("[MF-VMM]-- PROCESSINSTANCEID=" + pinstid);
                System.out.println("[MF-VMM]-- FOLDERID=" + folderId);
                ...
                System.out.println("[MF-VMM]-- TEMPFLAG--FOR-" + pinstid + "--" + dataMap.get("TEMPFLAG").toString());
                System.out.println("[MF-VMM]-- UPLOADFLAG--FOR-" + pinstid + "--" + dataMap.get("UPLOADFLAG").toString());
                System.out.println("[MF-VMM]-- TEMPDOC_LOCATION--FOR-" + pinstid + "--" + dataMap.get("TEMPDOC_LOCATION").toString());
                if (pinstid != null && folderId != 0L) {
                    System.out.println("[MF-VMM]-- Data Fetched Going to initiate Doc Upload");
                    ...
                    if (dataMap.get("TEMPFLAG").toString().equals("")) {
                        ...
                        System.out.println("[MF-VMM]--PROCEEDING FOR DOCUPLOAD.");
                        int Status = initiateDocUpload(dataMap, folderId, con, pinstid);
                        System.out.println("[MF-VMM]-- Status:" + Status);
                         if (Status == 1) {
                            try {
                                Set<String> foundDocument = new HashSet<>();
                                String idtype = null;
                                String customerID = dataMap.get("CUSTOMERID").toString();
                                System.out.println("[MF-VMM]--CIFID: " + customerID);
                                String directoryPath = "DMS_ARCHIVAL" + File.separator + "MF" + File.separator + "SB_CKYC" + File.separator + "EXT-" + customerID;
                                System.out.println("[MF-VMM]--LOCAL PATH:DMS_ARCHIVAL" + File.separator + "MF" + File.separator + "SB_CKYC" + File.separator + "EXT-" + customerID);
                                File directory = new File(directoryPath);
                                try {
                                    mf_pinstid = String.valueOf(dataMap.get("PROCESSINSTANCEID"));
                                    System.out.println("[MF-VMM]-- SELECTING ID PROOF TYPE FOR PINSTID: " + mf_pinstid);
                                    ...
                                    try (PreparedStatement cpr = con.prepareStatement(curretn_case)) {
                                        ...
                                        try (ResultSet idrs = cpr.executeQuery()) {
                                            while (idrs.next()) {
                                                ...
                                                System.out.println("[MF-VMM]--Work-item live status: " + pinstid_present);
                                    ...
                                    try (PreparedStatement pid = con.prepareStatement(idq)) {
                                        ...
                                        try (ResultSet idrs = pid.executeQuery()) {
                                            while (idrs.next()) {
                                                ...
                                                System.out.println("[MF-VMM]--ID_PROOF_NUMBER: " + ID_PROOF_NUMBER);
                                                dataMap.put("ID_PROOF_NUMBER", ID_PROOF_NUMBER);
                                            }
                                        }
                                    }
                                } catch (SQLException e) {
                                    System.out.println("[MF-VMM]--ERROR OCCURED WHILE TRYING TO FETCH THE PRIMARY-ID TYPE");
                                ...
                                System.out.println("[MF-VMM]--PRIMARYIDPROOF_TYPE: " + idprooftype);
                                String[] documentNames;
                                switch (idprooftype) {
                                    case "1":
                                        System.out.println("[MF-VMM]--PRIMARY ID IS AADHAR");
                                        documentNames = new String[]{"Photograph", "Signature", "UID"};
                                        break;
                                    case "2":
                                        System.out.println("[MF-VMM]--PRIMARY ID IS VOTER");
                                        documentNames = new String[]{"Photograph", "Signature", "Voters_Identity_Card"};
                                        break;
                                    case "3":
                                        System.out.println("[MF-VMM]--PRIMARY ID IS Pancard");
                                        documentNames = new String[]{"Photograph", "Signature", "Pancard"};
                                        break;
                                    case "4":
                                        System.out.println("[MF-VMM]--PRIMARY ID IS Driving Licence");
                                        documentNames = new String[]{"Photograph", "Signature", "Driving_License"};
                                        break;
                                    case "6":
                                        System.out.println("[MF-VMM]--PRIMARY ID IS Passport");
                                        documentNames = new String[]{"Photograph", "Signature", "Passport"};
                                        break;
                                    default:
                                        documentNames = new String[]{"Photograph", "Signature", "UID"};
                                        System.out.println("[MF-VMM]--ID TYPE IS NEITHER 1 NOR 2NOW WE CONSIDER PRIMARY ID IS AADHAR");
                                        break;
                                }
                                ...
                                if (!missingDocuments.isEmpty()) {
                                    ...
                                    try {
                                        System.out.println("[MF-VMM]--MANDATORY DOCUMENTS MISSING");
                                        sftpstatus = 3;
                                    } catch (Exception e) {
                                        e.printStackTrace();
                                    }
                            ...
                            try {
                                ...
                                switch (sftpstatus) {
                                    case 1: {
                                        ...
                                        String uquery = "UPDATE MF_RU_DMSARCHIVAL_VMM SET EXECUTION_FLAG = ?,REMARKS = ?,DOCS_IN_SFTP = ?,PROCESSED_AT = SYSDATE WHERE PROCESSINSTANCEID = ?";
                                    ...
                                    case 3: {
                                        ...
                                        String uquery = "UPDATE MF_RU_DMSARCHIVAL_VMM SET EXECUTION_FLAG = ?,REMARKS = ? ,MISSING_DOCS=?,DOCS_IN_SFTP = ?,PROCESSED_AT = SYSDATE WHERE PROCESSINSTANCEID = ?";
                                        try (PreparedStatement upstmt = con.prepareStatement(uquery)) {
    ...
    public int initiateDocUpload(Map dataMap, Long folderId, Connection con, String pinstid) {
        ...
        try {
            ...
            System.out.println("[MF-VMM]-- DMS Ejb Object Created.");
            System.out.println("[MF-VMM]-- DMS-IP=" + this.configObj.getDmsIp());
            System.out.println("[MF-VMM]-- DMS-Port=" + this.configObj.getDmsPort());
            NodeFolder nf = new NodeFolder();
            nf.setUuid(folderId);
            System.out.println("[MF-VMM]-- - sessionId-" + this.sessionId);
            ...
            if (!this.sessionId.equals("")) {
                ...
                System.out.println("[MF-VMM]-- Document Details Fetched.");
                if (!nodeDoc.isEmpty()) {
                    for (NodeDocument nodeD : nodeDoc) {
                        ...
                        System.out.println("[MF-VMM]--Documentname: " + dataMap.get("DocName") + " Extension: " + dataMap.get("EXT"));
                        ...
                        System.out.println("[MF-VMM]-- - Going to extract Docs.");
                        String docExtName = dataMap.get("DocName").toString();
                        System.out.println("[MF-VMM]-- - docExtName---" + docExtName);
                        ...
                        System.out.println("[MF-VMM]--DOCNAME AFTER MODIFICATION: " + dataMap.get("ProofName"));
                        System.out.println("[MF-VMM]-- PROCEEDING FOR FILE CREATION");

                        status = fileOperation(is, dataMap, con);
                        is.close();
                    }
                    System.out.println("[MF-VMM]-- For Loop Stopped");
                    System.out.println("[MF-VMM]-- All Doc Extracted Initiating DocMerge");
                    try {
                        cKycupdate(dataMap, con, pinstid);
                    } catch (Exception ex) {
                        updateFlag(con, dataMap.get("PROCESSINSTANCEID").toString(), "EXC", ex.getMessage());
                        System.out.println("[MF-VMM]-- UPDATED MF_RU_DMSARCHEIVAL: EXCEPTION OCCURED FOR:" + dataMap.get("PROCESSINSTANCEID").toString());
                        System.out.println("[MF-VMM]-- Inside Exception -CAUSED BY: "+  ex.getCause());
                        System.out.println("[MF-VMM]-- Inside Exception - " + ex);
                        ex.printStackTrace();
                    }
                } else {
                    status = 0;
                    System.out.println("[MF-VMM]-- The List nodeDoc is empty");
                    System.out.println("[MF-VMM]-- Sending status= 0");
                }
            } else {
                System.out.println("[MF-VMM]-- Session Not Found To Upload Docs");
            }
        } catch (InvalidRepositoryException | com.servo.dms.exception.PathNotFoundException | IOException ex) {
            updateFlag(con, dataMap.get("PROCESSINSTANCEID").toString(), "EXC", ex.getMessage());
            System.out.println("[MF-VMM]-- UPDATED MF_DOCEXE_SFTP: EXCEPTION OCCURED FOR:" + dataMap.get("PROCESSINSTANCEID").toString());
            ex.getCause();
            System.out.println("[MF-VMM]-- EX -" + ex);
    ...
    public int fileOperation(ByteArrayInputStream is, Map<String, String> dataMap, Connection con) {
        ...
        try {
            ...
            if (!file.exists()) {
                ...
                System.out.println("[MF-VMM]-- - Base Directory Created.");
            }
            ...
            if (!file.exists()) {
                file.mkdir();
                System.out.println("[MF-VMM]-- - Child Folder Created.");
            ...
            System.out.println("[MF-VMM]---- Extension found- " + extension);
            System.out.println("[MF-VMM]---- DocumentName- " + docName);
            String docExtName = dataMap.get("ProofName");
            String DocTypeName = dataMap.get("CsvName");
            System.out.println("[MF-VMM]---- File opertaion docExtName- " + docExtName);
            System.out.println("[MF-VMM]---- File opertaion csvname- " + DocTypeName);
            ...
            System.out.println("[MF-VMM]-- EKYC FLAG: " + dataMap.get("EKYCFLAG").toString());
            if (dataMap.get("EKYCFLAG").equalsIgnoreCase("Y")) {
                System.out.println("[MF-VMM]--SKIPPING ID PROOF'S WHILE WRITTING.");
                if (extension.equalsIgnoreCase("pdf")) {
                    ...
                    if (!DocTypeName.trim().startsWith("ID Proof")) {
                        System.out.println("[MF-VMM]-- Inside the if loop" + DocTypeName);
                ...
                System.out.println("[MF-VMM]-- Doc Copied To Server Locations--" + DocTypeName);
...
//            System.out.println("[MF-VMM]-- Doc Copied To Server Locations--" + DocTypeName);
            ...
            System.out.println("[MF-VMM]-- Csv Entry Created");
            status = 1;
        } catch (IOException ex) {
            updateFlag(con, dataMap.get("PROCESSINSTANCEID"), "ERR", ex.getMessage());
            System.out.println("[MF-VMM]-- File writing exe - " + ex);
            ex.printStackTrace();
            return status;
        }
        return status;
    ...
    public void cKycupdate(Map<String, String> dataMap, Connection con, String pinstid) {
        ...
        try {
            ...
            System.out.println("[MF-VMM]-- HashCustProof---" + HashCustProof.toString());
            String IdProoftype = HashCustProof.get("IDPROOFTYPE").toString();
            System.out.println("[MF-VMM]-- IdProoftype---" + IdProoftype);
            String AddressProofType = HashCustProof.get("ADDRESS_PROOF_TYPE").toString();
            System.out.println("[MF-VMM]-- AdressProoftype---" + AddressProofType);
            System.out.println("[MF-VMM]-- Completed the CKYC Documents Changes");
            File file = new File("DMS_ARCHIVAL" + File.separator + "MF" + File.separator + "SB_CKYC" + File.separator + "EXT-" + dataMap.get("CUSTOMERID"));
            System.out.println("[MF-VMM]-- CKyc-- File-" + file.getAbsolutePath());
            this.configObj.setDmsSftpLocalDir("EXT-" + dataMap.get("CUSTOMERID"));
            System.out.println("[MF-VMM]-- TEMPLOC-- SFTP-" + this.configObj.getDmsSftpLocalDir());
            dataMap.put("TEMPLOC", this.configObj.getDmsSftpLocalDir());
            String csv = file.toString() + File.separator + "EXT-" + dataMap.get("CUSTOMERID") + ".csv";
            writer = new CSVWriter(new FileWriter(csv, true), '|', CSVWriter.NO_QUOTE_CHARACTER);
            System.out.println("[MF-VMM]-- CKyc-- csv file-" + csv);

            List<String> files = new ArrayList<>();
            List<String> HashDocName = new ArrayList<>();

            System.out.println("[MF-VMM]--DOCUMENT NAMES FOR : " + pinstid);
            try {
                HashDocName = getDocumentsName(con, pinstid, dataMap);
                System.out.println("[MF-VMM]--DOCUMENT NAMES FETCHED:");
            } catch (Exception e) {
                System.out.println("[MF-VMM]--ERROR WHILE FETCHING DOCUMENT NAMES:");
            }

            String UID_Present = "NO";

            if (!HashDocName.isEmpty()) {
                System.out.println("[MF-VMM]--DocumentNameList is not empty.");
                ...
                System.out.println("[MF-VMM]--DocumentNameList is empty.");
            }

            System.out.println("[MF-VMM]--IS UID PRESENT IN THE DocumentNameList : " + UID_Present);
            ...
            for (String strTemp : arrSplit) {
                ...
                System.out.println("[MF-VMM]--Status---" + StatusValue);
...
//                        } else 
                ...
                if (testFile.exists()) {
                    files.add(fileA);
                    files.add(fileB);
                    System.out.println("[MF-VMM]---fileA--" + fileA);
                    System.out.println("[MF-VMM]---fileB--" + fileB);
                    System.out.println("[MF-VMM]--Pdf upload csvname: " + csvName);
                    ...
                    System.out.println("[MF-VMM]-- test");
                }
            }
            writer.close();
        } catch (FileNotFoundException fx) {
            System.out.println("[MF-VMM]--Updating File not found");
    ...
    public static void ImageToPDF(List<String> inputFiles, String outputFile) {
        try {
            ...
            System.out.println("[MF-VMM]-- - C Kyc Doc Write Exc- " + e);
        }
    }
    ...
    public void initiateSftp(propertyConfig configObj, Connection con, Map dataMap) {
        ...
        try {
            ...
            System.out.println("[MF-VMM]-- - Local Dir -" + localDirectory + fileToFTP);
            System.out.println("[MF-VMM]-- - check if the file exists");
            String filepath = localDirectory + fileToFTP;
            File file = new File(filepath);
            if (!file.exists()) {
                throw new RuntimeException("[MF-VMM]-- Error. Local file not found");
            }
            System.out.println("[MF-VMM]-- - Initializes the file manager");
            manager.init();
            System.out.println("[MF-VMM]-- - Setup our SFTP configuration");
            FileSystemOptions opts = new FileSystemOptions();
            SftpFileSystemConfigBuilder.getInstance().setStrictHostKeyChecking(opts, "no");
            SftpFileSystemConfigBuilder.getInstance().setUserDirIsRoot(opts, true);
            SftpFileSystemConfigBuilder.getInstance().setTimeout(opts, Integer.valueOf(10000));
            password = URLEncoder.encode(password);
            System.out.println("[MF-VMM]-- - Create the SFTP URI using the host name, userid, password,  remote path and file name");
            String sftpUri = "sftp://" + userId + ":" + password + "@" + serverAddress + "/" + remoteDirectory + fileToFTP;
            System.out.println("[MF-VMM]-- --" + sftpUri);
            System.out.println("[MF-VMM]-- - Create local file object");
            FileObject localFile = manager.resolveFile(file.getAbsolutePath());
            System.out.println("[MF-VMM]-- - Create remote file object");
            FileObject remoteFile = manager.resolveFile(sftpUri, opts);
            System.out.println("[MF-VMM]-- -- Copy local file to sftp server");
            remoteFile.copyFrom(localFile, Selectors.SELECT_SELF);
            System.out.println("[MF-VMM]-- -- Folder upload successful");
            ...
            for (File item : subFiles) {
                ...
                System.out.println("[MF-VMM]-- -- " + file.getAbsolutePath());
                ...
                if (item.isFile()) {
                    ...
                    System.out.println("[MF-VMM]-- -- File upload successful");
                }
            }
            cSftp.chmod(511, remoteDirectory + "/" + dataMap.get("TEMPDOC_LOCATION").toString());
            System.out.println("[MF-VMM]-- -- Updating Flag.");
            if (updateFlag(con, dataMap.get("PROCESSINSTANCEID").toString(), "UPD", "")) {
                System.out.println("[MF-VMM]-- Routing Done for - " + dataMap.get("PROCESSINSTANCEID").toString());
                ...
                File directoryToDelete = new File(deletedirectoryPath, folderToDelete);
                if (directoryToDelete.exists()) {
                    System.out.println("[MF-VMM]--Directory Found -- Deleting Existing Folder After upload to sftp: EXT-" + deletecustomerID);
                    deleteDirectory(directoryToDelete);
                } else {
                    System.out.println("[MF-VMM]--Directory does not exist.");
                }
            }
        } catch (JSchException | com.jcraft.jsch.SftpException | RuntimeException | org.apache.commons.vfs.FileSystemException ex) {
            updateFlag(con, dataMap.get("PROCESSINSTANCEID").toString(), "EXC", ex.getMessage());
            System.out.println("[MF-VMM]-- UPDATED MF_RU_DMSARCHEIVAL: EXCEPTION OCCURED FOR:" + dataMap.get("PROCESSINSTANCEID").toString());
            System.out.println("[MF-VMM]-- --File upload Ex - " + ex);
        } finally {
            manager.close();
            channel.disconnect();
            session.disconnect();
    ...
    public boolean updateFlag(Connection con, String pinstId, String type, String tempLocation) {
        ...
        try {
            if (type.equalsIgnoreCase("Copy")) {
                ...
                query = "UPDATE MF_RU_DMSARCHIVAL_VMM SET EXECUTION_FLAG = ? ,PROCESSED_AT = SYSDATE WHERE PROCESSINSTANCEID = ?";
                pstmt = con.prepareStatement(query);
                pstmt.setString(1, "UD");
                pstmt.setString(2, pinstId);
            } else if (type.equalsIgnoreCase("ERR")) {
                query = "UPDATE MF_RU_DMSARCHIVAL_VMM SET EXECUTION_FLAG = ?,REMARKS = ? ,PROCESSED_AT = SYSDATE WHERE PROCESSINSTANCEID = ?";
                pstmt = con.prepareStatement(query);
                pstmt.setString(1, "E");
                pstmt.setString(2, tempLocation);
                pstmt.setString(3, pinstId);
            ...
            System.out.println("[MF-VMM]-- -- UPdate Status Eex - " + ex);
    ...
    public Map getDocuemntsDetails(Connection con, String strPinstid, Map dataMap) {
        ...
        try {
            ...
            try (PreparedStatement cpr = con.prepareStatement(curretn_case)) {
                ...
                try (ResultSet idrs = cpr.executeQuery()) {
                    while (idrs.next()) {
                        ...
                        System.out.println("[MF-VMM]--Work-item live status TO FETCH THE IDPROOF TYPE: " + pinstid_present);
            ...
            System.out.println("[MF-VMM]--QUERY TO FETCH THE IDPROOF TYPE: " + ddq);
...
//            }
    ...
    public List getDocumentsName(Connection con, String strPinstid, Map dataMap) {
        ...
        try {
            ...
            System.out.println("[MF-VMM]--WORK ITEM LIVE STATUS TO FETCH THE DOCUMENT NAMES: " + live_status);
            ...
            while (rs.next()) {
                ...
                System.out.println("[MF-VMM]--DOCUMENTS PRESENT FOR THE PINSTID : " + strPinstid + ": " + nbsName);
                docnames.add(nbsName);
                // Here you can add the fetched data to your ArrayLinkedList or perform any other operations
            }
            closeConnection(rs, pstmt);
        } catch (SQLException ex) {
            System.out.println("[MF-VMM]--Exception occured while fetching Document names. " + ex);
            ex.printStackTrace();
            return docnames;
        } finally {
            closeConnection(rs, pstmt);
